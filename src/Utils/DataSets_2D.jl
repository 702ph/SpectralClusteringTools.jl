...
"""
    make_circles(num_classes::Int, num_points_per_class::Int, noise::Float64=0.0, adjust_scale::Bool=true, return_labels::Bool=true)
...
- `return_labels`: If true, returns the points with their associated labels; if false, returns the points with an array of ones.
...
# Example
`points, labels = make_circles(3, 1000, 0.1, false, true)`
"""
function make_circles(num_classes::Int, num_points_per_class::Int, noise::Float64=0.0, adjust_scale::Bool=true, return_labels::Bool=true)
    ...
    if return_labels
        ...
        return normalized_points, ones(Int, length(labels))
    end
end

"""
    make_spirals_2d(num_points_per_class::Int, noise::Float64=0.0, return_labels::Bool=true)
...
# Example
`points, labels = make_spirals_2d(500, 0.1, true)`
"""
function make_spirals_2d(num_points_per_class::Int, noise::Float64=0.0, return_labels::Bool=true)
    points = Matrix{Float64}(undef, 0, 2)
    labels = Vector{Int}(undef, 0)
    ...
    if return_labels
        ...
        return points, ones(Int, length(labels))
    end
end

"""
    make_blobs_2d(num_classes::Int, num_points_per_class::Int, noise::Float64=0.0, return_labels::Bool=true)
...
# Example
`points, labels = make_blobs_2d(5, 500, 0.5, true)`
"""
function make_blobs_2d(num_classes::Int, num_points_per_class::Int, noise::Float64=0.0, return_labels::Bool=true)
    points = Matrix{Float64}(undef, 0, 2)
    labels = Vector{Int}(undef, 0)
...
"""
make_moons(n_samples::Int=300, noise::Float64=0.05, return_labels::Bool=true)
    ...
    # Example
    ```julia
    X, labels = make_moons(500, 0.1, false)
    scatter(X[1,:], X[2,:], group=labels, title="Mixed Moons Dataset")
"""

function make_moons(n_samples::Int=300, noise::Float64=0.05, return_labels::Bool=true)
    ...
    labels = vcat(fill(1, size(X1, 1) + size(Mix1, 1)), fill(2, size(X2, 1) + size(Mix2, 1)))
